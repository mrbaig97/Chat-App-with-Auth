...
exports.deleteMessageForMe = async (req, res, next) => {
    ...
    try {
        await updateMessages({ _id: messageId, sender: userId }, { isDeleted: true });

        generateResponse(null, "Message deleted for me", res);
    } catch (error) {
        next(new Error(error.message));
    }
}
...
exports.deleteMessageForEveryone = async (req, res, next) => {
    ...
    try {
        ...
        generateResponse(null, "Message deleted for everyone", res);
    } catch (error) {
        next(new Error(error.message));
    }
}
